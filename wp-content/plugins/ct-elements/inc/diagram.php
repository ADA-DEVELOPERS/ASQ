...
add_action( 'widgets_init', 'ct_diagram_register_widgets');
function ct_diagram_register_widgets() {
	if ( !is_blog_installed() )
		return;
		register_widget('CT_Widget_Diagram');
}




class CT_Widget_Diagram extends WP_Widget {
	...
	function widget( $args, $instance ) {
		...
		ct_build_diagram($title, $instance['summary'], $instance['type'], $instance['skills'], true, $instance['effects_enabled'], $style);
		echo '</div>';
		echo '</div>';
		echo $after_widget;
	}
	...
	function get_skill_code($id, $data=null) {
		...
		?>
			<fieldset style="border: 1px dashed #DFDFDF; padding: 0 5px; margin: 0 0 10px 0;">
				<p><label for="skill_<?php echo esc_attr($id.'_'.$this->get_field_id('title')); ?>"><?php _e('Title:', 'ct'); ?></label>
				...
				<p><label for="skill_<?php echo esc_attr($id.'_'.$this->get_field_id('amount')); ?>"><?php _e('Amount (in percents):', 'ct'); ?></label>
				...
				<div><label for="skill_<?php echo esc_attr($id.'_'.$this->get_field_id('color')); ?>"><?php _e('Color:', 'ct'); ?></label>
...
?>
		...
		<p><label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title:', 'ct'); ?></label>
		<input class="widefat" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" type="text" value="<?php echo esc_attr($title); ?>" /></p>

		<p><label for="<?php echo esc_attr($this->get_field_id('summary')); ?>"><?php _e('Summary:', 'ct'); ?></label>
		...
		<p><label for="<?php echo esc_attr($this->get_field_id('type')); ?>"><?php _e('Type:', 'ct'); ?></label>
		<select class="widefat" id="<?php echo esc_attr($this->get_field_id('type')); ?>" name="<?php echo esc_attr($this->get_field_name('type')); ?>">
			<option <?php if ($type == 'circle') echo 'selected="selected"'; ?> value="circle"><?php _e('Circle', 'ct'); ?></option>
			<option <?php if ($type == 'line') echo 'selected="selected"'; ?> value="line"><?php _e('Lines', 'ct'); ?></option>
		</select></p>
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('effects_enabled')); ?>"><?php _e('Lazy loading enabled', 'ct'); ?></label>
		</p>

		<p><?php _e('Skills', 'ct'); ?>:</p>
		...
		<script type="text/javascript">
			if (ct_init_diagram_edit != undefined)
...
add_action( 'admin_enqueue_scripts', 'ct_admin_diagram_scripts_init' );
function ct_admin_diagram_scripts_init() {
	wp_enqueue_script('ct-diagram-edit', get_template_directory_uri() . '/js/diagram_edit.js', array('jquery'));
}

function ct_build_diagram($title, $summary, $type, $skills, $is_widget, $effects_enabled, $style) {
	if ($type == 'line') {
		ct_build_line_diagram($title, $summary, $skills, $is_widget, $effects_enabled, $style );
	}
	if ($type == 'circle') {
		ct_build_circle_diagram($title, $summary, $skills, $is_widget, $effects_enabled, $style);
	}
}

function ct_build_line_diagram($title, $summary, $skills, $is_widget, $effects_enabled, $style  ) {
	wp_enqueue_script('ct-diagram-line');
?>
	<?php  if (!empty($title)): ?>
	...
	<div class="<?php if($effects_enabled): ?>lazy-loading lazy-loading-not-hide<?php endif; ?>" <?php if($effects_enabled): ?>data-ll-item-delay="0"<?php endif; ?>>
		<div class="digram-line-box <?php if($effects_enabled): ?>lazy-loading-item<?php endif; ?>" <?php if($effects_enabled): ?>data-ll-effect="action" data-ll-action-func="ct_start_line_digram"<?php endif; ?>>
...
function ct_build_circle_diagram($title, $summary, $skills, $is_widget, $effects_enabled, $style) {
	wp_enqueue_script('ct-diagram-circle');
	$max_font_size = 0;
	if(ct_get_option('h5_font_size')) {
		$max_font_size = ct_get_option('h5_font_size');
	} else if (ct_get_option('body_font_size')) {
		$max_font_size = ct_get_option('body_font_size');
	}
	?>
...
// Shortcode
function ct_diagram_shortcode($atts, $content) {
	...
	ob_start();
	ct_build_diagram($title, $summary, $type, $skills, false, $effects_enabled, $style);
...
function ct_skill_shortcode($atts) {
