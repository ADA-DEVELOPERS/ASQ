	...
	if($ct_item_data['icon_shape'] == 'hexagon'){
		switch ( $ct_item_data['icon_size'] ) {
			case 'small':
				...
				break;
			case 'medium':
				$ct_border_indent = '47.5px';
				$ct_min_height = '150px';
				break;
			case 'large':
				$ct_border_indent = '92.5px';
				$ct_min_height = '270px';
				break;
			case 'xlarge':
		...
		switch ( $ct_item_data['icon_size'] ) {
			case 'small':
				$ct_border_indent = '27px';
				$ct_min_height = '100px';
				break;
			case 'medium':
				$ct_border_indent = '42px';
				$ct_min_height = '150px';
				break;
			case 'large':
				$ct_border_indent = '82px';
				$ct_min_height = '270px';
				break;
			case 'xlarge':
				$ct_border_indent = '122px';
				$ct_min_height = '320px';
				break;
			}
	}
...
<div id="post-<?php the_ID(); ?>" <?php if($params['effects_enabled']) echo ' data-ll-finish-delay="200" '; ?> <?php post_class(array( 'quickfinder-item', $quickfinder_item_rotation, $ct_quickfinder_effect, $ct_item_data['icon_size'], $params['effects_enabled'] ? 'lazy-loading' : '')); ?>>
	...
	<?php if($quickfinder_style != 'vertical' || $quickfinder_item_rotation == 'even') : ?>
		<div class="quickfinder-item-info-wrapper" style="min-height: <?php echo $ct_min_height ?>;">
			<?php if($quickfinder_style == 'vertical-3') : ?>
				<div class="connector" style="border-color: <?php echo esc_attr($connector_color); ?>; <?php echo ($params['alignment'] == 'left' ? 'right' : 'left'); ?>: -<?php echo $ct_border_indent;?>;">
				</div>
			<?php endif; ?>
