...
function ct_importStatus(maindata) {
	data = {};
	data['action'] = 'ct_get_ocdi_import_status';
	data['main_action'] = maindata.get('action');
	$.ajax({
		method: 'GET',
		url: ocdi.ajax_url,
		data: data
	}).done( function( response ) {
		if($('.ct-ocdi-status-message').length == 0) {
			$('<div class="ct-ocdi-status-message"></div>').insertAfter($( '.js-ocdi-ajax-loader' ));
		}
		var $messageBox = $('.ct-ocdi-status-message').eq(0);
		$messageBox.html(response.message);
	});

	$( document ).on( 'ocdiImportComplete', function() {
		$('.ct-ocdi-status-message').remove();
	});
}