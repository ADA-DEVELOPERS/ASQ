...
class CT_Mega_Menu_Walker extends Walker_Nav_Menu {
	...
	private function get_item_data($item_id) {
		global $CT_Mega_Menu_Default;

		if (!isset($this->items_data[$item_id])) {
			$data = get_post_meta( $item_id, '_menu_item_ct_mega_menu', true );
			$this->items_data[$item_id] = array_merge($CT_Mega_Menu_Default, (array) $data);
		}
		$this->ct_mobile_clickable = get_post_meta( $item_id, '_menu_item_ct_mobile_clickable', true );
	...
	private function init_menu_logo($args) {
		...
		if ( !isset( $this->menu_logo_inited ) ) {
			...
			$logo_position = cryption_get_option('logo_position', 'left');
	...
	private function print_logo(&$output) {
		...
		$header_params = cryption_get_sanitize_page_header_data($page_id);
		$header_light = $header_params['header_menu_logo_light'] ? '_light' : '';
		$output .= '<li class="menu-item-logo">' . cryption_print_logo($header_light, false) . '</li>';
	}
