...
<?php
	if($style) {
		...
		$return_html = '<div class="ct-gmaps ct-wrapbox ct-wrapbox-style-'.$classes.'" style="'.$css_style.'">'.
			'<div class="ct-wrapbox-inner">'.
			($style == '11' ? '<div class="gmaps_round">' : '').
			'<div class="ct-gmaps-hide"><iframe style="'.$frame_css_style.' border:0;" scrolling="no" marginheight="0" marginwidth="0" src="' . esc_url($link . '&amp;t=' . $type . '&amp;z=' . $zoom . '&amp;output=embed' . $bubble) . '" class="ct-wrapbox-element"></iframe>'.($disable_scroll ? '<a class="map-locker" href="javascript:void(0);"></a>' : '').'</div>'.
			($style == '11' ? '</div>' : '').
			'</div>'.
			'</div>';
		echo $return_html;
...
<div class="<?php echo esc_attr($css_class); ?>">
	...
	<div class="wpb_wrapper">
		<div class="wpb_map_wraper <?php if ($hide_title) echo 'hide_title'?>">
			...
			if ( preg_match( '/^\<iframe/', $link ) ) {
				if($disable_scroll) {
					$link = preg_replace( '/^\<iframe/', '<iframe style="pointer-events: none; border:0; width: 100%;"', $link ).'<a class="map-locker" href="javascript:void(0);"></a>';
				}
				echo $link;
			}
			else echo '<iframe height="' . $size . '" scrolling="no" marginheight="0" marginwidth="0" src="' . esc_url($link . '&amp;t=' . $type . '&amp;z=' . $zoom . '&amp;output=embed' . $bubble) . '"'.($disable_scroll ? ' style="pointer-events: none; width: 100%"' : ' style="width: 100%"').'></iframe>'.($disable_scroll ? '<a class="map-locker" href="javascript:void(0);"></a>' : '');
			?>
		</div>
