...
while ( $my_query->have_posts() ) {
	...
	if ( $link != 'link_no' ) {
		if ( $link == 'link_post' ) {
			$link_image_start = '<a class="link_image" href="' . esc_url(get_permalink( $post_id )) . '" title="' . sprintf( esc_attr__( 'Permalink to %s', 'cryption' ), the_title_attribute( 'echo=0' ) ) . '">';
...
if ( $teasers ) {
	...
	$teasers = esc_html__( 'Nothing found.', 'cryption' );
}
...
$output .= '<div class="wpb_gallery_slides' . esc_attr($type) . '" data-interval="' . esc_attr($interval) . '"' . $flex_fx . '>' . $teasers . '</div>';
