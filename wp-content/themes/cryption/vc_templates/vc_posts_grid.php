		...
		<div class="wpb_wrapper">
			...
			<div class="teaser_grid_container">
				<?php if ( 'yes' === $filter && ! empty( $this->filter_categories ) ) :
					...
					esc_html_e( 'All', 'cryption' );
					echo '</a></li>';
					foreach ( $this->getFilterCategories() as $cat ) :
						echo '<li><a href="#"'
						     . ' data-filter=".grid-cat-<?php echo esc_attr($cat->term_id); ?>">'
						     . esc_attr( $cat->name )
						     . '</a></li>';
					endforeach;
					echo '</ul>';
				...
				<?php endif ?>
				<ul class="wpb_thumbnails wpb_thumbnails-fluid vc_clearfix" data-layout-mode="<?php echo esc_attr($grid_layout_mode); ?>">
					<?php
					/**
					 * Enqueue js/css
					 * {{
					...
					<?php if ( count( $posts ) > 0 ) :  ?>
						...
						<?php endforeach ?>
					<?php else : ?>
						<li class="<?php echo esc_attr($this->spanClass( 1 )); ?>"><?php esc_html_e( 'Nothing found.', 'cryption' ) ?></li>
					<?php endif ?>
				</ul>
			</div>
		</div>
